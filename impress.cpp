...
int main(int argc, char * argv[]) {
    ...
    Random dciRand=Random(deseeder());
    srand(deseeder());
    ...
    init();
    /* ****************************************************/
    double gevk=0.463215;
    double gevsigma=89.102;
    double gevMu=601.542;
    double gammaA=0.572057;
    double gammaB=17.7348;
    long int capacity=IMP_input->FSused;
    DedupChunkInfoGenerator dciGenerator=DedupChunkInfoGenerator(
    		dciRand);
    DedupChunkInfoGenerator * dcigP;
    dcigP=&dciGenerator;
    dcigP->openDB();
    dcigP->loadTokenToMap();
   // dcigP->generateDedupChunkList();
    ...
    */
    ...
    for (int i=0; i<IMP_input->Numfiles; i++) {
        ...
        if(IMP_input->Flag[sf_constraint] == 1) {
        	...
        	}while(data1>1024*1024*1024);
        }
        cout<<"file size is "<<data1/(1024*1024)<<"MB"<<endl;
...
//********************************************************************************************
       //here to create file
        ...
        total_file_size += make_generic_file(parent_path,data1, depth, fp_log,dcigP);
        //
