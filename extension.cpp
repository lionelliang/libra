 ...
 */
/*
 * This file has been modified by Longxiang Wang
 * the make_dedup_ascii_file has been added to evaluate our method
...
extern struct timeval main_time_start, main_time_end, dirtree_start,
		dirtree_end, filecontent_start, filecontent_end, filesize_start,
		filesize_end, word_model_start, word_model_end, extension_start,
		extension_end, aging_start, aging_end, file_creation_start,
		file_creation_end;

extern double main_time_total, dirtree_total, filecontent_total,
		word_model_total, extension_total, aging_total, file_creation_total,
		filesize_total;
...
char extension_array[][4] = { "nul", "txt", "jpg", "exe", "cpp", "htm", "h__",
		"dll", "gif", "pdb", "pst", "pch", "mp3", "lib", "wma", "vhd", "pdf",
		"mpg", "tal" };
char extension_header[][100] = { "", "", "FFD8\n", "", "", "", "", "",
		"GIF89a\n", "", "", "", "", "", "", "", "%PDF-1.4\n", "", "" };
float extension_popularity_large[] = { 7.79, 5.02, 1.45, 2.08, 4.88, 3.24, 9.32,
		3.64, 3.43, 1.94, 0.00, 0.10, 0.16, 1.76, 0.19, 0.01, 0.04, 0.00, 0.00 };
float extension_popularity_default[] = { 6.91, 3.64, 3.08, 2.88, 3.26, 4.45,
		6.69, 6.63, 6.77, 1.34, UNKNOWN, UNKNOWN, UNKNOWN, 1.46, UNKNOWN,
		UNKNOWN, UNKNOWN, UNKNOWN, LONG_TAIL };
int TOTAL_POPULARITY = 0;
extern int ACTUAL_LOG_CREATION;
//float extension_popularity[NUM_EXTENSIONS];
float * extension_popularity;

void make_dedup_ascii_file(char * filepath, long double size,
		DedupChunkInfoGenerator * dcigP);
 ...
 **************************************************** */
long double make_generic_file(char * filepath, long double size, int depth,
		FILE * fp_log, DedupChunkInfoGenerator * dcigP) {
	...
	if (IMP_input->Actualfilecreation == 1) {
		...
		case CPP:
		case H:
			//cout<<"call make_dedup_ascii_file()"<<endl;
			//make_ascii_file(full_filepath, size, -1);
			make_dedup_ascii_file(full_filepath, size, dcigP);
			break;
		case GIF:
			...
			sprintf(cmd, "%s/%s %s %d %d", EXT_HELPERS, "gif", full_filepath,
					1000, 1000);
			size = 1757;
			if (access(helperexec, X_OK) == 0)
				...
				print_debug(0,
						"EXT_HELPER: gif not available, resorting to binary file\n");
				make_binary_file(full_filepath, size, -1);
			}
			__Ntime gettimeofday(&file_creation_end, NULL);
			__Ntime file_creation_total += diff_time(file_creation_start,
					file_creation_end);
			break;
		case MP3:
			...
			__Ntime gettimeofday(&file_creation_start, NULL);
			sprintf(args,
					"-a \"Test Artist\" -A \"Test Album\" -t \"Test Song Title\" -c \" Impressions File MP3\" -g \"POP\" -y \"2007\" -T \"1\"");
			sprintf(helperexec, "%s/%s", EXT_HELPERS, "mp3");
			sprintf(cmd, "%s/%s %s %s", EXT_HELPERS, "mp3", args,
					full_filepath);
			if (access(helperexec, X_OK) == 0)
				system(cmd);
			else {
				print_debug(0,
						"EXT_HELPER: mp3 not available, resorting to binary file\n");
			}
			__Ntime gettimeofday(&file_creation_end, NULL);
			__Ntime file_creation_total += diff_time(file_creation_start,
					file_creation_end);
		...
		case JPG:
			__Ntime gettimeofday(&file_creation_start, NULL);
			sprintf(helperexec, "%s/%s", EXT_HELPERS, "gif");
			sprintf(cmd, "%s/%s %s %d %d", EXT_HELPERS, "gif", "temp.gif", 1000,
					1000);
			size = 1757;
			if (access(helperexec, X_OK) == 0) {
				system(cmd);
...
int make_binary_file(char * filepath, long double size, int extension_num) {
	...
	if (IMP_input->Layoutscore < 1) {
		...
		if (num_blocks >= min_blocks_needed) {
			num_seeks = (int) (
					(fragment_degree * num_blocks
							- floor(fragment_degree * num_blocks) <= 0.5) ?
							floor(fragment_degree * num_blocks) :
							ceil(fragment_degree * num_blocks));
			if (num_seeks != 0) {
				frag_schedule_array = (long double*) malloc(
						sizeof(long double) * num_seeks);
#ifdef PRINT
			...
			for (int i = 0; i < num_seeks; i++) {
				frag_schedule_array[i] = (long double) (rand()
						% ((int) num_blocks) + 1);
				print_debug(0, " - %Lf -", frag_schedule_array[i]);
			}

			qsort((void*) frag_schedule_array, (size_t) num_seeks,
					(size_t) sizeof(long double), compfunc);
	...
	if ((fp = fopen(filepath, "wb")) == NULL) {
		...
		if (extension_num > 0) {
			temp_write = (double) fwrite(
					(void*) extension_header[extension_num],
					(size_t) (sizeof(extension_header[extension_num])), 1, fp);
			written += (long double) temp_write
					* sizeof(extension_header[extension_num]);
		}
		...
		while (written < size) {
			...
			if (size - written < MY_BLOCK_SIZE) {
				temp_write = (long double) fwrite((void*) buf,
						(size_t) (size - written), 1, fp);
				written += temp_write * (size - written);
			} else {
				temp_write = (long double) fwrite((void*) buf,
						(size_t) (MY_BLOCK_SIZE), 1, fp);
			...
			if (num_seeks != 0 && seeker < num_seeks) {
				...
				while (frag_schedule_array[seeker]
						== frag_schedule_array[seeker + 1])
...
/* ****************************************************
 ...
 Function that generates an deduplicated ascii file use the statistic model.
...
void make_dedup_ascii_file(char * filepath, long double size,
	...
	int remainSize;
	//char * chunkBuf;
	cout<<"generating file: "<<filepath<<endl;
	do {
		string token=dcigP->getTokenBlock();
		pos_write(fd, (char *)token.data(), token.length());
		char space=dcigP->getSpace();
		pos_write(fd, &space, 1);
...
int make_ascii_file(char * filepath, long double size, int extension_num) {
	...
	if ((fd = pos_creat(filepath, mode)) < 0) {
		...
		// if filetype has a header, write it, e.g, PDF
		if (extension_num > 0) {
			written += (long double) pos_write(fd,
					(void*) extension_header[extension_num],
					sizeof(extension_header[extension_num]));
		}
		...
		while (written < size) {
			...
			if (IMP_input->Flag[sf_wordfreq] >= 1) {
				...
				random_word_block(buf);
				__Ntime gettimeofday(&filecontent_end, NULL);
				__Ntime filecontent_total += diff_time(filecontent_start,
						filecontent_end);
			}
			...
			if (size - written < MY_BLOCK_SIZE) {
				...
				written += (long double) pos_write(fd, (void*) buf,
						(size_t) (size - written));
